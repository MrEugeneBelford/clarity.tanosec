...
export default function FullReport({ action }: { action?: "print" | "pdf" }) {
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      ...
      // Force PDF export mode for consistent theming and layout
  ...
  return (
    <main className="flex min-h-svh w-full flex-col items-center p-4 md:p-8 font-body">
      <div id="full-report" ref={reportRef} className="w-full max-w-5xl space-y-5">
        {/* Branding Header */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <Card className="lg:col-span-1">
            ...
            <CardContent className="text-center space-y-3">
              <p className="text-6xl font-bold">{scorePct}%</p>
          ...
          <Card className="lg:col-span-2">
            ...
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {Object.entries(data.categoryScores).map(([catId, scores]) => {
                ...
                return (
                  <div key={catId} className="flex items-center gap-3 rounded-md border p-3" style={{ breakInside: "avoid" }}>
        ...
        <section className="space-y-3">
          ...
          {prioritized.low.length > 0 ? (
            prioritized.low.map((rec, i) => (
              <div key={i} className="p-3 rounded-md border bg-card flex items-start gap-3" style={{ breakInside: "avoid" }}>
